...
@dataclass
class SpiritThought:
    """A narrative thought from the Spirit."""
    narration: str
    criticism: str
    guidance: str
    memories: list[str]
...
class SpiritAgent(AbstractAgent):
    ...
    def __init__(
        ...
        # Thought stream (shared with Brain)
        ...
        self._on_command: Optional[Callable[[SpiritThought], None]] = None
    ...
    def set_thought_stream(self, stream: asyncio.Queue[SpiritThought]) -> None:
        """Set the thought stream for sharing thoughts with Brain."""
    ...
    def on_command(self, callback: Callable[[SpiritThought], None]) -> None:
        """Register callback for thoughts."""
    ...
    def get_command_queue(self) -> Optional[asyncio.Queue[SpiritThought]]:
        """Get the thought stream for Brain to consume."""
    ...
    async def _process_signal(self, signal: Signal) -> None:
        ...
        if not parsed:
            self.logger.warning("Failed to parse Spirit response, attempting correction", module="spirit")
...
Please provide a corrected, valid JSON response for Spirit analysis of this signal.

The JSON must contain: narration, criticism, guidance, memories (array), reflection.
...
Respond with valid JSON only."""
            ...
            try:
                ...
                if not parsed:
                    self.logger.error("Failed to parse corrected Spirit response", module="spirit")
                    return
                self.logger.info("Successfully corrected malformed JSON response", module="spirit")
            except Exception as e:
                self.logger.error(f"Spirit correction failed: {e}", module="spirit")
                return
        ...
        # Always send a narrative thought to Brain if we have guidance
        guidance = parsed.get("guidance", "")
        if guidance:
            # Add recent conversation context for Brain to maintain chat memory
            recent_messages = self.get_history()[-6:]  # last 6 messages (3 exchanges)
            if recent_messages:
                context_lines = []
            ...
            if self._thought_stream:
                ...
                await self._thought_stream.put(thought_obj)
                self.logger.info(f"Sent thought to Brain: {guidance[:50]}...", module="spirit")
    ...
    async def _process_commands(self, commands: list[dict], thought: str = "") -> None:
        ...
        for cmd_data in commands:
            ...
            if cmd_type == "remember":
                ...
                # Send thought to shared stream for Brain
                if self._thought_stream:
                    await self._thought_stream.put(command)
                    self.logger.info(
                        f"Thought shared: {content[:50]}...",
                        module="spirit"
                    )
